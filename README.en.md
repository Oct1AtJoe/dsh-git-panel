...
- **Write bar** (top of the panel, under the tabs):
  ...
  - **Stash changes / Restore stash**: `git stash push` (optional message) / `git stash pop`. **This is NOT the same as staging (`git add`) above** — it follows git's own terminology so the two cannot be confused
  - **Status**: shows a plain-language summary (`Working tree clean` / `N changed · A staged · B unstaged`); click to force a re-read. The raw `git status --porcelain` text stays available in the hover tooltip
