...
const DICTS: Record<Lang, Dict> = {
  zh: {
    ...
    'write.stash': '储藏改动',
    'write.stashPop': '恢复储藏',
    'write.stash.tip': '把当前所有未提交的改动收进储藏区（git stash push），工作区回到干净状态。这与「暂存」（git add）不是一回事。',
    'write.stashPop.tip': '把储藏区里最近一份改动倒回工作区（git stash pop）。恢复后该份储藏即被移除。',
    'write.status': '状态',
    'write.status.tip': '重新读取 git status 并刷新下方变更列表（不影响工作区内容）',
    'write.status.clean': '工作区干净',
    'write.status.summary': '{total} 个文件已改动 · {staged} 已暂存 · {unstaged} 未暂存',
    'write.status.conflicts': '（含 {count} 个冲突）',
    'op.ok': '操作成功',
    'op.refresh': '刷新',
  ...
  en: {
    ...
    'write.stash.tip': 'Park every uncommitted change in the stash (git stash push), leaving a clean working tree. This is NOT the same as staging (git add).',
    ...
    'write.status': 'Status',
    'write.status.tip': 'Re-read git status and refresh the change list below (does not touch the working tree)',
    'write.status.clean': 'Working tree clean',
    'write.status.summary': '{total} changed · {staged} staged · {unstaged} unstaged',
    'write.status.conflicts': ' ({count} conflicting)',
    'op.ok': 'Done',
    'op.refresh': 'Refresh',
  ...
  es: {
    ...
    'write.commit.generated': 'Mensaje generado: revísalo antes de confirmar',
    ...
    'write.commit.noModel': 'No hay un modelo predeterminado disponible, así que no se puede generar el mensaje. Elige uno en ajustes.',
    'write.push': 'Empujar',
    'write.sync': 'Sincronizar',
    'write.stash': 'Guardar cambios',
    'write.stashPop': 'Restaurar guardado',
    'write.stash.tip': 'Guarda todos los cambios sin confirmar en el stash (git stash push) y deja el árbol de trabajo limpio. NO es lo mismo que preparar (git add).',
    'write.stashPop.tip': 'Devuelve al árbol de trabajo el guardado más reciente (git stash pop). Esa entrada se elimina al restaurarla.',
    'write.status': 'Estado',
    'write.status.tip': 'Vuelve a leer git status y actualiza la lista de cambios de abajo (no toca el árbol de trabajo)',
    'write.status.clean': 'Árbol de trabajo limpio',
    'write.status.summary': '{total} modificado(s) · {staged} preparado(s) · {unstaged} sin preparar',
    'write.status.conflicts': ' ({count} en conflicto)',
    'op.ok': 'Hecho',
    ...
    'changes.batchFailed': 'Falló tras {count} archivo(s): {reason}',
