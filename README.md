...
- **写操作条**（面板顶部，位于标签页下方）：
  ...
  - **自动生成提交信息**：输入框右端内部的星形按钮会读取**暂存区（staged）**的变更内容，连同仓库最近几条提交的主题一起交给当前默认模型，生成一条符合本仓库语言与提交前缀习惯的信息并回填输入框（输入框已有文字时直接覆盖）。暂存区为空时给出明确提示且不触发生成；生成失败时给出失败提示，**输入框里已写的内容原样保留**
  - **推送**：一键 `git push` 当前分支
